...
const commandFiles = fs.readdingSyrc(commandsPath).filter(file => file.endsWith(".js"))
...
client.once(Events.ClientReady, c => {
	console.log('Pronto! O seu login foi realizado como ${c.user.tag}' )
});
client.login(TOKEN)
...
client.on(Events.InteractionCreate, async interaction =>{
    if(interaction.isStringSelectMenu()){
        ...
        if (selected == "javascript"){
             await interaction .reply("Documentções do JavaScript: https://developer.mozilla.org/en-US/docs/Web/JavaScript")
    ...
    if(!interaction.isChatInputCommand()) return
        ...
        if (!command) {
            console.error("Command não encontrado")
            return
        }
        try {
